...
public class ThreadTest4 {
    ...
    private static void countPrimesWithExecutor(int numberOfTasks) {
        ...
        for (int i = 0; i < numberOfTasks; i++) {
            ...
            /**
             * create temporary final variables out of min and max, since non final
             * local varialbe can't be accessed inside lambda
             */
            final int tmpMin = min;
            final int tmpMax = max;
            // create a lambda
            Callable <Integer> CountPrimesTaskLambda = () -> { return countPrimes(tmpMin, tmpMax); };
            // pass lamda to submit
            Future<Integer> oneResult = executor.submit(  CountPrimesTaskLambda );
